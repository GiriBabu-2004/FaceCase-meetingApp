  ...
  return (
    <div className='flex flex-col items-center'>
        <div className='relative'>
          <Image src={slides[currentSlide].image} alt='meeting_feature' width={300} height={300} className='rounded-full w-40 h-40 md:w-64 md:h-64'/>
          <Button variant='ghost' size='icon' className='absolute top-1/2 -left-10 transform -translate-y-1/2 -translate-x-full' onClick={prevSlide}>
            <ChevronLeft className='w-6 h-6'/>
          </Button>
          <Button variant='ghost' size='icon' className='absolute top-1/2 -right-10 transform -translate-y-1/2 translate-x-full' onClick={nextSlide}>
            <ChevronRight className='w-6 h-6'/>
          </Button>
        </div>
        <h2 className='text-2xl font-semibold mt-6 text-gray-900 dark:text-white mb-2'>
        ...
        <div className='flex justify-center space-x-2 mt-4'>
          {slides.map((_,index) =>(
            <div key={index} className={`w-3 h-3 rounded-full ${index == currentSlide? "bg-blue-600" : "bg-gray-300"}`}>
